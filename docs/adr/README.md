...
| 21 | [The name wraps, and the grid caps at two columns](0021-the-name-wraps-and-the-grid-caps-at-two-columns.md) | A task's name is never truncated: the overview row is two lines, and the desktop grid caps at two columns rather than three. | [#73](https://github.com/stainii/task/issues/73) |
| 22 | [A min/max round starts when you closed it, not when it fired](0022-a-min-max-round-starts-when-you-closed-it.md) | `MinMax` counts its interval from the day the last task was closed, so ticking off a chore you were late with buys a full interval of quiet. `Calendar` is untouched. | [#75](https://github.com/stainii/task/issues/75) |
