...
**Companion ledger.** This ledger covers *portal* features. Defects in `task`'s own already-migrated code are in [`docs/repo-health.md`](repo-health.md) (#18) and are cited here as **D1–D7** / **F1–F3** (equivalently **HEALTH-01…08**). Both feed #11; #16 checks both.

**Coverage is checked, not assumed.** [`docs/coverage-gate.md`](coverage-gate.md) is #16's traceability table — every row here, with its verdict, the backlog issue that carries it, and whether the verdict is still true. **A verdict here can be stale**: six rows have been overturned or reworded by a later design ticket, five of them because an ADR deleted the concept the row is named after. When an ADR deletes a named thing, grep this file and the triage tables for that name.
...
| TODO-010 | **Create tasks from a template** `POST /api/task/from-template/` | `TaskController.createFromTemplate` | **LANDED** (moved) | Now `POST /api/task-templates/{id}/tasks`, in the `template` module, via an in-process `TaskCreationRequestedEvent`. *(Rebuilt by [#50](https://github.com/stainii/task/issues/50): the event is `TaskTemplateFired` and rendering moved onto the aggregate, so the endpoint now renders through the same code `/render-fixtures/` pins.)* |

| TODO-011 | **Patch a task** `PATCH /api/task/{id}` — server fills taskId/flowId from the path | `TaskController.patch` | **PARTIAL** | Now `POST /api/task-patches` with the taskId in the body, and it is the **only** write verb: the first patch for a task id creates it. The 400 landed in [#46](https://github.com/stainii/task/issues/46) (**D5**). The response still returns nothing — the client already knows what it sent, and hears the durable version back on the stream. |
| TODO-012 | **Fetch patches since a timestamp** `GET /api/task/patch/?since=` — the reconnect/catch-up call | `TaskPatchController.getAllTaskPatchesSince` | **LANDED** (merged) | Folded into the SSE endpoint: `GET /api/task-patches` replays then tails. `?since=` is a **sequence** since [#46](https://github.com/stainii/task/issues/46), presented with its `epoch`; querying the client-minted timestamp never delivered a patch written offline before the reader's cursor. |
| TODO-013 | **Tail patches over SSE** `GET /api/task/patch/?tail` | `TaskPatchController.tail` + `TaskPatchSseEmitterService` | **LANDED** | Same mechanism; `task` adds named events + heartbeats. Portal's `// TODO: find a way to write an integration test for SSE` was answered by [#10](https://github.com/stainii/task/issues/10) (`StepVerifier.thenCancel()`), and the nginx worry is ADR-0007's, not the application's. Since [#46](https://github.com/stainii/task/issues/46) the server honours `Last-Event-ID`, ends the stream on a bounded lifetime, and resyncs a cursor it cannot serve. |
...
| TODO-018 | **SSE emitter registry** — register listeners, drop dead emitters | `services/TaskPatchSseEmitterService` | **LANDED** | Reimplemented as `TaskPatchSseEmitterService` + `service/helper/SseEmitters`. Portal's version had a latent bug (`List.remove(Collection)`); `task`'s does not. [#46](https://github.com/stainii/task/issues/46) took the id off the heartbeat — stamping one clobbered the browser's `Last-Event-ID` with a value naming no patch. |
...
| TODO-021 | **DeviationBase** START_DATE / DUE_DATE | `model/template/DeviationBase.java` | **LANDED** | Verbatim. *(#12's verdict of **keep as landed** is **overturned to drop** by [ADR-0013](adr/0013-one-anchor-and-a-trigger-that-shapes-the-form.md): one anchor with two offsets replaces both `*DeviationBase` columns, measured across 11 real definitions where the base does nothing in ten and is silently wrong in the eleventh. Recorded on [#47](https://github.com/stainii/task/issues/47), tallied by [#16](https://github.com/stainii/task/issues/16).)* |
...
| TODO-024 | **Template CRUD** `GET/POST/PUT/DELETE /api/template/` | `controllers/TaskTemplateController` + `services/TaskTemplateService` | **LANDED** | `/api/task-templates`, plus existence checks and typed exceptions portal did not have. *(Rebuilt by [#50](https://github.com/stainii/task/issues/50): D4's four fixes, `@Valid` bodies, deactivate/reactivate endpoints, and a delete that is refused once the template has tasks.)* |
...
| Id | Capability | Where it lives | Status in `task` | Notes / fidelity |
|---|---|---|---|---|
| TODO-042 | **Application bootstrap** — `@SpringBootApplication`, `@EnableMongoRepositories`, sets default TimeZone to `Europe/Brussels`, `APPLICATION_NAME = "Todo"` | `PortalTodoApplication.java` | **PARTIAL** | `TaskBackEndApplication` exists; **the timezone default is not set anywhere in `task`**, and `LocalDate`/`LocalDateTime` fields now depend on the JVM default. Worth a row of its own at triage. **Closed by [#44](https://github.com/stainii/task/issues/44)**: the zone is `task.time-zone` in `application.yml`. |
| TODO-043 | **Jackson configuration** — a `Clock` bean, lenient deserialization, JavaTime/Jdk8 modules, dates as ISO strings | `ModuleConfiguration.java` | **PARTIAL** | `task` configures only `fail-on-null-for-primitives: false`. **No `Clock` bean** — `task` calls `LocalDate.now()` / `LocalDateTime.now()` directly, which is why time-dependent logic (D1) is awkward to test. **Closed by [#44](https://github.com/stainii/task/issues/44)**: the `Clock` bean is `config/TimeConfig`, every `now()` in `src/main` goes through it, and `LocalDate.now()` no longer compiles. |
...
| TODO-047 | **`TaskNotFoundException` (404)** | `exceptions/TaskNotFoundException.java` | **PARTIAL** | Exists and *did* carry `@ResponseStatus(404)` — this note's "nothing maps it to a status code" was wrong, corrected by [#46](https://github.com/stainii/task/issues/46), which also deletes the type: the 404 case is now `OrphanPatchException`, and the advice that had never existed does the mapping. **D5**. |
| TODO-048 | **`TaskPatchNotFoundException` (404)** | `exceptions/TaskPatchNotFoundException.java` | **PARTIAL** | Idem, and it has no survivor at all: patches are never fetched by id, so nothing can fail to find one. |
...
| REC-002 | **Execution** — a dated record that the task was done | `model/Execution.java` | **LANDED** | `recurring/Execution`; `long` id → `UUID`, and note **D6** (the `addExecution` copy-on-write workaround). *(Corrected by [#16](https://github.com/stainii/task/issues/16): #13's verdict of **keep as landed — the full execution log stays** does not survive [ADR-0001](adr/0001-one-task-aggregate-with-triggered-templates.md), which **deletes `Execution`**. The capability survives — *when did I last actually do this* is the newest `completedOn` among the template's completed tasks — so read the verdict as **transform**, owned by [#47](https://github.com/stainii/task/issues/47). The verdict's clause "D6 … carried as-is" is likewise void: HEALTH-06 records that the workaround dies with the class. #13's tally moves from 10 keep / 5 transform / 15 drop to **9 / 6 / 15**.)* |
...
| REC-004 | **CRUD** `GET/POST/PUT/DELETE /api/recurring-task/`, `/{id}` | `controllers/RecurringTaskController` | **PARTIAL** | `/api/recurring-task-templates` — but inconsistent routing/verbs and no service layer (**D4**). *(Built by [#50](https://github.com/stainii/task/issues/50) as one CRUD surface over the merged aggregate: explicit verbs, no trailing slashes, service layer, `@Valid`. D4 is closed.)* |

| REC-005 | **Register an execution** `POST /api/recurring-task/{id}/execution/`, publishing an execution event when the source is USER | `RecurringTaskController.addExecution` + `RecurringTaskService.addExecution` | **PARTIAL** | Endpoint landed. The event publication is gone (no bus), and with it the **USER vs EVENT source distinction** (`dtos/Source`) that prevented echo loops. *(Corrected by [#33](https://github.com/stainii/task/issues/33): there is **no successor endpoint**. [ADR-0011](https://github.com/stainii/task/blob/main/docs/adr/0011-completion-is-a-task-fact-the-template-reads.md) makes the out-of-band completion **client-minted** through the patch outbox, because "I already did this" must work offline. Read the verdict as **transform into a client affordance**, owned by [#36](https://github.com/stainii/task/issues/36) — not a kept endpoint. The date portal's `ExecutionDto` carried survives as `completedOn` on `Task`.)* |
| REC-006 | **Overdue scheduler** — daily cron: publish a reminder event for tasks hitting their *min* date and a final warning for those hitting *max*, cancelling the earlier reminder | `schedulers/PublishOvertimeRecurringTasks` | **PARTIAL** | `recurring/scheduler/CreateDueTasks` runs the same daily cron but **creates a task directly** instead of publishing a reminder. Two behaviours lost: the *escalation* (min → urgent-at-max, with cancellation of the superseded reminder) and the `urgent` flag. Also **D1**: `Period.getDays()` means anything with `min > 30` never fires at all. *(Built by [#49](https://github.com/stainii/task/issues/49): the collapse to one task created at `min` and due at `max` is the trigger's `defaultDueDateFor`, and D1 died with the class in [#47](https://github.com/stainii/task/issues/47).)* |
| REC-007 | **Event-driven execution** — an END event on this deployment's flowId registers an execution (this is how completing a todo ticked off the recurring task) | `messaging/EventListener` | **MISSING** | The loop *recurring task → task → done → execution recorded* is broken: `task` creates the task (REC-006) but nothing writes the execution back when it is completed. **The single most load-bearing gap in this service.** *(Closed by [#49](https://github.com/stainii/task/issues/49) without an event: `taskTemplateId` on the task is the provenance link, and the firing predicate reads the closure back through `TaskOccurrences` — the loop is a query, not a message.)* |
...
| REC-013 | **Service layer** — findAll / findById / create / update / delete / addExecution | `services/RecurringTaskService` | **MISSING** | `task` has **no service layer here at all**; the controller talks to the repository (**D4**). *(Built by [#50](https://github.com/stainii/task/issues/50): the controller talks to `TaskTemplateService` and nothing else, which `docs/quality-bar.md` §6 now states as a project-wide rule.)* |
...
| SOC-004 | **Register a contact** `POST /api/person/{id}/contact/` — stores `latestUpdates` and adds an execution to the person's recurring task | `PersonController.addContact`, `PersonService.addContact` | **MISSING** | *(Void, as the row already implied: a person is a template in the `social` context, and there is no execution endpoint to add to. The affordance is [#60](https://github.com/stainii/task/issues/60)'s client-minted completion.)* |
...
| RES-013 | **Front-end build and serve config** — `angular.json`, `proxy.conf.json`, `nginx-custom.conf`, `ngsw-config.json` (service worker), `cert.pem`/`key.pem`, `tslint.json` | `portal-front-end/` | **`ngsw-config.json` matters**: portal had an Angular service worker, i.e. real PWA/offline installability. **Built by #62** — `task-front-end` now has a service worker, a manifest and its own icon set. `assetGroups` carried over; `dataGroups` is deliberately empty. |
