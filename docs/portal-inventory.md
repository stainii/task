...
| FE-002 | **Client-side patch construction** — diff updated vs original task into a patch with a client-minted GUID | `todo/task-patch.service.ts` | **MISSING** | Note this is where portal minted **client-side patch ids** — the capability **D3** says `task`'s API no longer permits. |
...
| FE-005 | **Task typing** — focus / goals / fit-in / back-burner buckets from importance + due-date proximity (< 7 days) | `todo/task.model.ts` (`hasType*`) | **MISSING** | Present in the model; no view in this listing uses it. |
...
| FE-008 | **Complete a task** (from the panel, via a patch) | `todo/todo-task-panel/`, `todo/task.service.ts` | **MISSING** | |
| FE-009 | **Undo** — delete a patch | `todo/task.service.ts` (`undo`) | **MISSING** | Blocked by **D3** even if built. |
...
| DB-003 | **`portal-setlist`**, **`portal-health`**, **`portal-social-recurring-tasks`** (Postgres) | identical to DB-002. | Names inferred from the `deployment-name` mechanism, not from committed config — **verify against the live server before migrating**. Row counts unknown; this ledger was compiled from source, no database was queried. |
| DB-004 | **`portal-social`** (Postgres 5438, Liquibase) | `person(id bigint pk, name varchar(255) not null, color_thumbnail varchar(255) not null (renamed from image_name), sepia_thumbnail varchar(255), recurring_task_id bigint not null, latest_updates text)`; sequence `person_id_sequence` (increment 50, initial 0). | `recurring_task_id` is a **cross-database foreign key with no constraint** — it points into DB-003's social deployment. Migration has to resolve it. |
...
| CON-006 | **Keycloak** — realm-based auth; `portal.auth.keycloak-uri` on the back ends, `keycloak-js` on the front end | OIDC bearer tokens | all | **PARTIAL** — `task` is an OAuth2 resource server against realm `portal-realm`; whether the *existing realm and users* carry over is the map's open auth question. |
...
| RES-010 | **`portal-email`** | `portal-email/` | Explicitly conditional on the map: in scope only if new functionality needs it. No row until then. |
| RES-011 | **README files** | `portal-*/README.md` | Not read for this ledger; may contain deployment facts. |
...
1. **No database was queried.** Row counts, actual database names (DB-003) and how much real data exists per table are unknown. The data-migration ticket needs a live inventory that this ledger cannot supply.
...
3. **READMEs were not read** (RES-011). They may contradict or extend the deployment facts here.
4. **The four recurring-tasks deployments are inferred** from the `deployment-name` mechanism and the front-end constants (`housagotchi`, `setlist`, `health`, `social-recurring-tasks`). Only the housagotchi datasource is in committed config.
