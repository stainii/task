...
>
> **Reconfirmed by [#27](https://github.com/stainii/task/issues/27) (2026-08-05):** a full `verify` finished in **45:45** at **74% mutation / 87% test strength / 85% line coverage** (235 mutations, 174 killed, 35 uncovered) — a point above #20's figures, after removing `micrometer-registry-prometheus` and `spring-modulith-starter-insight`. That session re-derived this amendment instead of reading it, and killed a build on the superseded "cannot finish" claim. **Read this file first.**
...
**Other notes**: `lombok.config` is present, and Lombok is used heavily (`@Data`, `@Builder`, `@SneakyThrows`) — in tension with the map's stated preference for records and less Lombok. `error_prone.version` is declared as a property in `pom.xml` but **no Error Prone dependency or plugin uses it**; it is dead config. > **Resolved by [#10](https://github.com/stainii/task/issues/10) (2026-08-06).** The dead property was deleted by #20, and #10 has now installed the real thing: Error Prone 2.47.0 + NullAway 0.12.7, gating `src/main` on every build. The observation mattered more than it looked: [#19](https://github.com/stainii/task/issues/19) named Error Prone as one of the three things the build gates on, so for the life of this map **one of the three stated gates did not exist**. Nor did the thing it was supposed to enforce - all 28 `package-info.java` files are `@NullMarked` and nothing in the build read one. On its first run it found 19 issues in 11 files, including **defect D1 itself** (`JavaPeriodGetDays`). See `docs/quality-bar.md`. `task-workspace/` is empty apart from the IntelliJ `.idea` config.
