...
| `task-front-end` — `npm run e2e` | **5 green** locally, including the whole offline contract (cold start from IndexedDB, outbox order, stream resume, the auth stall) — **but see below** |

**One qualification on the e2e row, recorded rather than reran away.** The five specs passed locally on this commit, but CI's Playwright job then failed on `outbox-drains-in-order.spec.ts:62` — on a diff touching only this document and one back-end test — and a rerun of the same job on the same commit went green. That is the same line and signature as [#69](https://github.com/stainii/task/issues/69), which closed earlier the same day as the fix for it. It is now [#71](https://github.com/stainii/task/issues/71). **So this gate's e2e evidence should be read as *green, on a suite with one known intermittent spec*** — and that spec is the offline-ordering contract, which is not a peripheral one. #71 is deliberately not wired to block [#17](https://github.com/stainii/task/issues/17); that is the author's call.

**#71 has since named the cause, and it moves this row rather than merely explaining it.** The flake was a client defect, not a timing guess in the test: `AuthService`'s Keycloak initialisation is awaited by both sync loops before either touches the network, `keycloak-js` gives its silent `check-sso` no timeout and no error path, and a radio lost while that iframe was in flight left the promise pending for the life of the page — so the device made **no requests at all**, which is what the failing run's trace shows for the whole 90 seconds. Both waits are bounded now and the intermittency has a fix rather than a rerun, so the qualification above is **retired**: read this gate's e2e row as green on a suite with no known intermittent spec. What the qualification bought is worth keeping, though — the rerun was the evidence, and reranning it away would have deleted it.
