...
/**
 * PROTOTYPE (issue #78, wayfinder map #76) — throwaway. Not for production.
 *
 * Floating bottom bar that cycles the `?variant=` query param so the three search/checkbox
 * variants on the templates page can be flipped through without leaving the route.
 */
@Component({
  selector: 'app-search-prototype-switcher',
...
export class SearchPrototypeSwitcher {
  ...
  protected readonly labels: Record<string, string> = {
    A: 'Composable (search narrows the checkbox view)',
    B: 'Search overrides the checkbox',
    C: 'Checkbox + an explicit "search everywhere" chip',
  };
