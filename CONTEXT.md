...
The description is the one term with a **different word on screen**: the overview row marks a task
that has one with `notes`, because *description* is the domain's word for the field and *notes* is
what a person calls the thing written in it
([ADR-0021](docs/adr/0021-the-name-wraps-and-the-grid-caps-at-two-columns.md)). One field, two
registers — deliberately, and recorded here so the pair does not drift into two concepts. A
description that is only whitespace counts as none, in the row and in the panel body alike.
